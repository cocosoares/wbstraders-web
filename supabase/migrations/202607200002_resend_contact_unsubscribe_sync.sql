-- Reconcile Resend global unsubscribes back into the local consent ledger.
-- A manual re-subscribe in Resend never creates local consent: only an explicit
-- customer opt-in through a WBStraders surface may do that.
...
revoke all on function public.record_resend_contact_event(text,text,text,text,boolean)
  from public, anon, authenticated;
grant execute on function public.record_resend_contact_event(text,text,text,text,boolean)
  to service_role;
